  ...
  return (
    <div className="chart-card overflow-hidden">
      ...
      <div style={{ height: `${chartHeightInPx}px` }}>
        <ResponsiveBar
          theme={{ textColor: '#fff', fontSize: 9 }}
          data={[
          ...
          tooltip={({ data }) => {
            return (
              <div className="p-2 bg-white rounded bg-opacity-80 text-gray-600 absolute">
                {Object.keys(data).map((key) => {
                  if (key === 'color' || key == 'short_key') {
                    return null;
                  }
                  ...
                  return (
                    <div className="flex items-center" key={key}>
                      ...
                      <p className="ml-2 whitespace-nowrap">{data[key]}</p>
                    </div>
                  );
                })}
              </div>
