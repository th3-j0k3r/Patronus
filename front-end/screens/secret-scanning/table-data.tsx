...
const TableDataView: FC<{
  data?: ScannerResponse;
  setPageNumber: Dispatch<SetStateAction<number>>;
  pageNumber: number;
}> = ({ data: defaultData, setPageNumber, pageNumber }) => {
  ...
  const { isValidating, data } = useSWR<ScannerResponse>(
    enteredTerm.trim().length
      ? apiRoutes.searchScannedResults('ss', searchBy, enteredTerm) +
          `&page_num=${pageNumber}`
      : '',
  );
          ...
          tableData?.results?.map((each, index) => {
            return (
              <tr key={each.scan_id}>
                ...
                <TableData>
                  <Link
                    href={`/secret-scanning/${each.scan_id}/${slugify(
                      each.project_name || 'na',
                    )}`}
                  >
                ...
                <TableData title={parseIssue(each.issue as any)?.tags || ''} />
